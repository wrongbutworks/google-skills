...
The following example demonstrates a complete, production-ready `main.tf` using the official modules to configure a Global External Application Load Balancer with three distinct backends (Serverless NEG for API, Google Cloud Storage bucket for static content, and Cloud Armor security policies):
